...
import requests, json, os, subprocess, shutil, time, random
import traceback
from difflib import SequenceMatcher

# moore.wot.eecs.northwestern.edu config:
...
# By default, it assumes that the file is in the current directory, but you can change it
# e.g., WAR_ABS_PATH = '/home/<netid>/search-api-test/search-api-1.0-SNAPSHOT.war'
WAR_ABS_PATH = os.path.dirname(os.path.realpath(__file__)) + '/search-api-1.0-SNAPSHOT.war'

ENV_VARS = {
  "JAVA_HOME": JAVA_HOME,
  "AWS_ACCESS_KEY_ID": "AKIA3ATZL6C66MMPTD6N",
  "AWS_SECRET_ACCESS_KEY": "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",
  "ELASTIC_SEARCH_HOST": "ssa-elasticsearch.stevetarzia.com",
...
class ScoreManager:
    def __init__(self):
        ...
        # run tomcat on a random user port, so as not to conflict with other students
        self.port = random.randint(8000, 1<<16)

    def localhost_path(self):
...
def main(war_path):
    ...
    valid, points, comments = manager.finalize()
    print(comments)
    if (valid):
        print("Total points awarded = %d" % (points),)
    else:
        print("A problem occurred in the tester")
...
def run_war(abs_file_path, manager):
    ...
    shutil.copy(abs_file_path, 'target/')
    os.chdir(target_dir)
    subprocess.run(['jar', 'xvf', abs_file_path])
    os.chdir("../..")
    shutil.copy(target_dir + "/META-INF/maven/edu.northwestern.ssa/search-api/pom.xml", ".")

    # rewrite port in pom.xml
    subprocess.Popen("sed s/8080/"+str(manager.port)+"/g pom.xml > pom.xml.new; mv pom.xml.new pom.xml", shell=True)
    ...
    if succ:
        proc = subprocess.Popen(MVN + " tomcat7:run-war",
                                env=ENV_VARS, shell=True)
        SLEEP_TIME = 20
        MSG_INTERVAL = 5
        for i in range(SLEEP_TIME, 0, -MSG_INTERVAL):
            print("waiting for the local Tomcat to start for %d more seconds..." % (i,))
            time.sleep(MSG_INTERVAL) # wait for the local host to start
...
def test_status_code(manager):
    for query in ["", "?count=5"]:
        results = request_and_check_timeout(manager.localhost_path() + API_PATH + query)
        if results is None:
    ...
    for ok_query in ["?query=hello", "?query=billie%20eilish&count=8&offset=10&language=en"]:
        results = request_and_check_timeout(manager.localhost_path() + API_PATH + ok_query)
        if results is None:
...
def test_queries_exact(manager):
    queries = [
      ...
      "?query=anaconda&date=2021-01-25",
      "?query=anaconda&date=2021-01-25&language=en",
      "?query=billie%20eilish&count=8&offset=200",
      "?query=taylor%20swift&count=2&offset=10&language=en&date=2021-01-19",
      "?query=first%20blood&count=5&date=2021-01-19",
      "?query=bench%20press&count=6&language=en",
      "?query=bitcoin%20ethereum%20litecoin&date=2021-01-25",
      "?query=social%20distancing&language=en&offset=250",
      "?query=stay%20at%20home&date=2021-01-19",
      "?query=Northwestern&offset=1024",
      "?query=2021-01-19&language=ru&date=2021-01-19",
      "?query=query&language=fi",
    ]
    for i in range(len(queries)):
        query = queries[i]
        test_response = request_and_check_timeout(manager.localhost_path() + API_PATH + query)
